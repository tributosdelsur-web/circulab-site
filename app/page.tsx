...
export default function Home() {
  ...
  const T: any = {
    es:{
      ...
      h1a:'Tu basura vale',h1b:'USD',h1c:'Tu palabra,',h1d:'historial crediticio',h1e:'Tu arte,',h1f:'capital hoy',
      ...
      verPanel:'Ver el Panel →',calcular:'🏢 Calculá el ahorro de tu consorcio',registrar:'Registrar residuo',
      consorcioCTA:'¿Administrás un consorcio?',consorcioDesc:'Tu consorcio ya paga por la basura. Calculá cuánto podría recibir por separar sus orgánicos con Circulab.',calcularBtn:'Calcular ahorro →',
      inversorCTA:'¿Sos inversor?',inversorDesc:'Accedé al One Pager confidencial de la Ronda Seed 2026. USD 500K Opción A · USD 2M Opción B.',onePagerBtn:'Ver One Pager →',
      whitepaperCTA:'¿Querés el detalle técnico?',whitepaperDesc:'Whitepaper completo con metodología dMRV, tokenómica OLV, fases de desarrollo y modelo financiero. Requiere firma de NDA.',whitepaperBtn:'Acceder al Whitepaper →',
      productoTag:'El producto existe',productoH2:'Así se ve tu identidad financiera digital',accederPanel:'Acceder al Panel completo →',simulador:'🏢 Simulador de ahorro',
      ecosistemaH2:'Ecosistema Ciudadano',ecosistemaDesc:'Tres verticales que se retroalimentan y generan un efecto de red único',
      equipoH2:'El equipo',
      comunidadH2:'Comunidad OLIVIA en acción',comunidadDesc:'Acciones climáticas reales verificadas por nuestra comunidad',verComunidad:'Ver toda la comunidad →',sinPosts:'Sé el primero en publicar una acción climática',
      encuestaH2:'¿Reciclás?',encuestaDesc:'Contanos cómo manejás tus residuos. Tu respuesta construye OLIVIA.',encuestaBtn:'Responder encuesta · 2 min →',
      ...
      footer:{panel:'Panel',simulador:'Simulador',registrar:'Registrar residuo',comunidad:'Comunidad',encuesta:'Encuesta',onepager:'One Pager',whitepaper:'Whitepaper',contacto:'Contacto'},
      copyright:'Circulab Tech © 2026 · Distrito IA CABA',
    },
    en:{
      tagline:'AI District · Knowledge Economy Law',
      h1a:'Your waste is worth',h1b:'USD',h1c:'Your word,',h1d:'credit history',h1e:'Your art,',h1f:'capital today',
      desc:'Circulab is a citizen trust engine. We validate acts of responsibility with AI to turn them into real assets.',
      verPanel:'View Dashboard →',calcular:'🏢 Calculate your building savings',registrar:'Register waste',
      consorcioCTA:'Do you manage a building?',consorcioDesc:'Your building already pays for waste. Calculate how much it could earn by separating organics with Circulab.',calcularBtn:'Calculate savings →',
      inversorCTA:'Are you an investor?',inversorDesc:'Access the confidential One Pager for Seed Round 2026. USD 500K Option A · USD 2M Option B.',onePagerBtn:'View One Pager →',
      whitepaperCTA:'Want the technical details?',whitepaperDesc:'Full whitepaper with dMRV methodology, OLV tokenomics, development phases and financial model. Requires NDA signature.',whitepaperBtn:'Access Whitepaper →',
      productoTag:'The product exists',productoH2:'This is your digital financial identity',accederPanel:'Access full Dashboard →',simulador:'🏢 Savings calculator',
      ecosistemaH2:'Citizen Ecosystem',ecosistemaDesc:'Three verticals that feed each other and generate a unique network effect',
      equipoH2:'The team',
      comunidadH2:'OLIVIA Community in action',comunidadDesc:'Real verified climate actions from our community',verComunidad:'View full community →',sinPosts:'Be the first to post a climate action',
      encuestaH2:'Do you recycle?',encuestaDesc:'Tell us how you manage your waste. Your answer builds OLIVIA.',encuestaBtn:'Answer survey · 2 min →',
      ...
      footer:{panel:'Dashboard',simulador:'Calculator',registrar:'Register waste',comunidad:'Community',encuesta:'Survey',onepager:'One Pager',whitepaper:'Whitepaper',contacto:'Contact'},
      copyright:'Circulab Tech © 2026 · AI District CABA',
    }
  }
    ...
    <main style={{background:bg,minHeight:'100vh',fontFamily:'system-ui,sans-serif',color:text,transition:'all 0.3s'}}>
      ...
      <nav style={{padding:'16px 32px',borderBottom:`1px solid ${border}`,display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,background:navBg,backdropFilter:'blur(10px)',zIndex:100,flexWrap:'wrap',gap:12}}>
        ...
        <div style={{display:'flex',gap:16,alignItems:'center',flexWrap:'wrap'}}>
          ...
          <a href="/comunidad" style={{fontSize:13,color:'#3b82f6',textDecoration:'none',fontWeight:600}}>🌿 {lang==='es'?'Comunidad':'Community'}</a>
          <a href="/onepager" style={{fontSize:13,color:'#f59e0b',textDecoration:'none',fontWeight:600}}>{t.nav.onepager}</a>
          <a href="/whitepaper" style={{fontSize:13,color:'#a855f7',textDecoration:'none',fontWeight:600}}>Whitepaper</a>
      ...
      <section style={{padding:'100px 32px',textAlign:'center',maxWidth:800,margin:'0 auto'}}>
        ...
        <p style={{fontSize:18,color:muted,lineHeight:1.6,marginBottom:40,maxWidth:600,margin:'0 auto 40px'}}>{t.desc}</p>
        <div style={{display:'flex',gap:12,justifyContent:'center',flexWrap:'wrap'}}>
          <a href="/dashboard" style={{background:'linear-gradient(135deg,#22c55e,#16a34a)',color:'white',padding:'14px 32px',borderRadius:12,fontSize:16,fontWeight:700,textDecoration:'none',boxShadow:'0 0 30px rgba(34,197,94,0.3)'}}>{t.verPanel}</a>
          <a href="/simulador" style={{background:'rgba(34,197,94,0.1)',border:'1px solid rgba(34,197,94,0.3)',color:'#22c55e',padding:'14px 32px',borderRadius:12,fontSize:16,fontWeight:700,textDecoration:'none'}}>{t.calcular}</a>
          <a href="/registrar" style={{background:dark?'rgba(255,255,255,0.04)':'rgba(0,0,0,0.04)',border:`1px solid ${border}`,color:text,padding:'14px 32px',borderRadius:12,fontSize:16,fontWeight:700,textDecoration:'none'}}>{t.registrar}</a>
        </div>
      </section>
      ...
      <section style={{padding:'60px 32px',background:dark?'rgba(34,197,94,0.04)':'rgba(34,197,94,0.06)',borderTop:'1px solid rgba(34,197,94,0.1)',borderBottom:'1px solid rgba(34,197,94,0.1)'}}>
        <div style={{maxWidth:800,margin:'0 auto',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:24}}>
          ...
          <a href="/simulador" style={{background:'linear-gradient(135deg,#22c55e,#16a34a)',color:'white',padding:'16px 32px',borderRadius:14,fontSize:15,fontWeight:700,textDecoration:'none',boxShadow:'0 0 30px rgba(34,197,94,0.25)',whiteSpace:'nowrap',flexShrink:0}}>{t.calcularBtn}</a>
        </div>
      </section>

      {/* Inversores CTAs */}
      <section style={{padding:'60px 32px',borderBottom:`1px solid ${border}`}}>
        <div style={{maxWidth:800,margin:'0 auto',display:'flex',flexDirection:'column',gap:16}}>
          {/* One Pager */}
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:24,background:dark?'rgba(245,158,11,0.04)':'rgba(245,158,11,0.06)',border:'1px solid rgba(245,158,11,0.15)',borderRadius:16,padding:'24px 28px'}}>
            <div>
              <div style={{fontSize:20,fontWeight:900,marginBottom:6,color:text}}>{t.inversorCTA}</div>
              <div style={{fontSize:14,color:muted,maxWidth:460}}>{t.inversorDesc}</div>
            </div>
            <a href="/onepager" style={{background:'linear-gradient(135deg,#f59e0b,#d97706)',color:'white',padding:'14px 28px',borderRadius:12,fontSize:14,fontWeight:700,textDecoration:'none',boxShadow:'0 0 20px rgba(245,158,11,0.25)',whiteSpace:'nowrap',flexShrink:0}}>{t.onePagerBtn}</a>
          </div>
          {/* Whitepaper */}
          <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:24,background:dark?'rgba(168,85,247,0.04)':'rgba(168,85,247,0.06)',border:'1px solid rgba(168,85,247,0.15)',borderRadius:16,padding:'24px 28px'}}>
            <div>
              <div style={{fontSize:20,fontWeight:900,marginBottom:6,color:text}}>{t.whitepaperCTA}</div>
              <div style={{fontSize:14,color:muted,maxWidth:460}}>{t.whitepaperDesc}</div>
              <div style={{display:'flex',gap:8,marginTop:10,flexWrap:'wrap'}}>
                {['📋 NDA digital','🤖 dMRV + IA','🪙 Tokenómica OLV','📊 Modelo financiero'].map(l=>(
                  <span key={l} style={{fontSize:11,color:muted,background:dark?'rgba(255,255,255,0.04)':'rgba(0,0,0,0.04)',padding:'3px 10px',borderRadius:20}}>{l}</span>
                ))}
              </div>
            </div>
            <a href="/whitepaper" style={{background:'linear-gradient(135deg,#a855f7,#9333ea)',color:'white',padding:'14px 28px',borderRadius:12,fontSize:14,fontWeight:700,textDecoration:'none',boxShadow:'0 0 20px rgba(168,85,247,0.25)',whiteSpace:'nowrap',flexShrink:0}}>{t.whitepaperBtn}</a>
          </div>
        </div>
      </section>
      ...
      <section id="producto" style={{padding:'80px 32px',maxWidth:1000,margin:'0 auto'}}>
        ...
        <div style={{textAlign:'center',display:'flex',gap:12,justifyContent:'center',flexWrap:'wrap'}}>
          <a href="/dashboard" style={{background:'linear-gradient(135deg,#22c55e,#16a34a)',color:'white',padding:'14px 32px',borderRadius:12,fontSize:15,fontWeight:700,textDecoration:'none',boxShadow:'0 0 30px rgba(34,197,94,0.3)',display:'inline-block'}}>{t.accederPanel}</a>
          <a href="/simulador" style={{background:dark?'rgba(255,255,255,0.04)':'rgba(0,0,0,0.04)',border:`1px solid ${border}`,color:text,padding:'14px 32px',borderRadius:12,fontSize:15,fontWeight:700,textDecoration:'none',display:'inline-block'}}>{t.simulador}</a>
        </div>
      </section>

      {/* Feed comunidad */}
      <section style={{padding:'80px 32px',background:dark?'rgba(255,255,255,0.01)':'rgba(0,0,0,0.02)',borderTop:`1px solid ${border}`,borderBottom:`1px solid ${border}`}}>
        <div style={{maxWidth:1000,margin:'0 auto'}}>
          <div style={{textAlign:'center',marginBottom:40}}>
            <div style={{fontSize:12,color:'#22c55e',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:12}}>🌿 OLIVIA</div>
            <h2 style={{fontSize:36,fontWeight:900,letterSpacing:'-0.02em',marginBottom:12,color:text}}>{t.comunidadH2}</h2>
            <p style={{fontSize:15,color:muted}}>{t.comunidadDesc}</p>
          </div>
          {posts.length===0?(
            <div style={{textAlign:'center',padding:'40px 0',color:muted,fontSize:14}}>{t.sinPosts}</div>
          ):(
            <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:16,marginBottom:32}}>
              {posts.map((post:any,i:number)=>(
                <div key={i} style={{background:card,border:`1px solid ${border}`,borderRadius:16,padding:'16px',display:'flex',flexDirection:'column',gap:10}}>
                  ...
                  {post.contenido&&<div style={{fontSize:13,color:text,lineHeight:1.5}}>{post.contenido}</div>}
                  {post.foto_url&&<img src={post.foto_url} alt="" style={{width:'100%',borderRadius:10,maxHeight:200,objectFit:'cover'}} />}
                  <div style={{fontSize:11,color:'#22c55e'}}>🌿 Acción climática verificada</div>
                </div>
              ))}
            </div>
          )}
          <div style={{textAlign:'center'}}>
            <a href="/comunidad" style={{background:'linear-gradient(135deg,#22c55e,#16a34a)',color:'white',padding:'14px 32px',borderRadius:12,fontSize:15,fontWeight:700,textDecoration:'none',display:'inline-block',boxShadow:'0 0 20px rgba(34,197,94,0.2)'}}>
              {t.verComunidad}
            </a>
          </div>
        </div>
      ...
      {/* Encuesta CTA */}
      <section style={{padding:'60px 32px',background:dark?'rgba(59,130,246,0.04)':'rgba(59,130,246,0.06)',borderBottom:`1px solid rgba(59,130,246,0.1)`}}>
        <div style={{maxWidth:800,margin:'0 auto',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:24}}>
          <div>
            <div style={{fontSize:24,fontWeight:900,marginBottom:8,color:text}}>{t.encuestaH2}</div>
            <div style={{fontSize:15,color:muted,maxWidth:500}}>{t.encuestaDesc}</div>
            <div style={{display:'flex',gap:12,marginTop:12,flexWrap:'wrap'}}>
              {['🌿 Sin juicio','⏱️ Solo 2 minutos','🔒 Anónima'].map(l=>(
                <span key={l} style={{fontSize:12,color:muted,background:dark?'rgba(255,255,255,0.04)':'rgba(0,0,0,0.04)',padding:'4px 10px',borderRadius:20}}>{l}</span>
              ))}
            </div>
          </div>
          <a href="/encuesta" style={{background:'linear-gradient(135deg,#3b82f6,#2563eb)',color:'white',padding:'16px 32px',borderRadius:14,fontSize:15,fontWeight:700,textDecoration:'none',boxShadow:'0 0 30px rgba(59,130,246,0.25)',whiteSpace:'nowrap',flexShrink:0}}>
            {t.encuestaBtn}
          </a>
        </div>
      </section>
      ...
      <footer style={{padding:'32px',borderTop:`1px solid ${border}`,textAlign:'center'}}>
        ...
        <div style={{display:'flex',gap:20,justifyContent:'center',flexWrap:'wrap',marginBottom:12}}>
          ...
          <a href="/comunidad" style={{fontSize:12,color:'#3b82f6',textDecoration:'none',fontWeight:600}}>{t.footer.comunidad}</a>
          <a href="/encuesta" style={{fontSize:12,color:'#3b82f6',textDecoration:'none'}}>{t.footer.encuesta}</a>
          <a href="/onepager" style={{fontSize:12,color:'#f59e0b',textDecoration:'none',fontWeight:600}}>{t.footer.onepager}</a>
          <a href="/whitepaper" style={{fontSize:12,color:'#a855f7',textDecoration:'none',fontWeight:600}}>{t.footer.whitepaper}</a>
          <a href="mailto:contacto@circulab.com.ar" style={{fontSize:12,color:muted,textDecoration:'none'}}>{t.footer.contacto}</a>
        </div>
        <div style={{fontSize:11,color:muted}}>contacto@circulab.com.ar · contacto@oliviacirculab.com.ar</div>
      </footer>
