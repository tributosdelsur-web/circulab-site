// SCRIPT 16 — Página /metamorfosis completa
// Similar en estructura a /ciudadano e /institucional
// Explica la vertical de residuos con su nombre correcto

const fs = require('fs');

const metamorfosisDir = 'app/metamorfosis';
if (!fs.existsSync(metamorfosisDir)) {
  fs.mkdirSync(metamorfosisDir, { recursive: true });
}

const metamorfosisPath = 'app/metamorfosis/page.tsx';

if (!fs.existsSync(metamorfosisPath)) {
  const pagina = `'use client'
import { useState, useEffect } from 'react'
import { supabase } from '../../lib/supabase'

export const metadata = {
  title: 'Metamorfosis · Residuos en créditos de carbono · OLIVIA Circulab',
  description: 'Metamorfosis es la vertical de residuos de Circulab Tech. Verificación dMRV con IA, OLV Verdes certificables por Verra VCS 2027. Primera infraestructura de datos ambientales ciudadanos de LATAM.',
}
...
`;
  ...
  fs.writeFileSync(metamorfosisPath, pagina);
  console.log('OK app/metamorfosis/page.tsx creado');
  console.log('URL: oliviacirculab.com.ar/metamorfosis');
} else {
  console.log('-- /metamorfosis: ya existe');
}

// Actualizar sitemap con /metamorfosis
if (fs.existsSync('public/sitemap.xml')) {
  console.log('-- sitemap.xml: ya incluye /metamorfosis (creado en script 14)');
} else {
  console.log('WARN: ejecutar script 14 primero para crear el sitemap');
}

console.log('');
console.log('Script 16 completado');
console.log('Páginas nuevas creadas:');
console.log('  /metamorfosis - vertical residuos completa');
console.log('  /privacidad - política Ley 25.326 Argentina');
console.log('  /reset-password - nueva contraseña');
console.log('  /auth/callback - handler Supabase auth');
console.log('  app/not-found.tsx - 404 personalizada');
