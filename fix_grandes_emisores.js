...
// ═══ 1. PÁGINA /grandes-emisores ═══
const dir = 'app/grandes-emisores';
...
fs.writeFileSync('app/grandes-emisores/page.tsx', page);
console.log('OK app/grandes-emisores/page.tsx creada');

// ═══ 2. AGREGAR 5TA PUERTA EN LANDING ═══
let landing = fs.readFileSync('app/page.tsx', 'utf8');
if (!landing.includes('/grandes-emisores')) {
  ...
  console.log('OK landing: 5ta puerta grandes emisores agregada');
} else {
  console.log('-- landing: ya tiene /grandes-emisores');
}

// ═══ 3. SITEMAP ═══
if (fs.existsSync('public/sitemap.xml')) {
  let sitemap = fs.readFileSync('public/sitemap.xml', 'utf8');
  if (!sitemap.includes('/grandes-emisores')) {
    sitemap = sitemap.replace('</urlset>',
      `  <url>
    <loc>https://oliviacirculab.com.ar/grandes-emisores</loc>
    <changefreq>weekly</changefreq>
    <priority>0.9</priority>
  </url>
</urlset>`);
    fs.writeFileSync('public/sitemap.xml', sitemap);
    console.log('OK sitemap actualizado');
  }
}

// ═══ 4. ACTUALIZAR INSTITUCIONAL · NARRATIVA GLOBAL ═══
let inst = fs.readFileSync('app/institucional/page.tsx', 'utf8');
if (!inst.includes('grandes-emisores') && !inst.includes('CORSIA') && !inst.includes('EU ETS')) {
  inst = inst.replace(
    `{/* MERCADO REGULATORIO */}`,
      ...
      <section style={{padding:'0 24px 32px',maxWidth:800,margin:'0 auto'}}>
        <div style={{background:'rgba(59,130,246,0.04)',border:'1px solid rgba(59,130,246,0.2)',borderRadius:16,padding:'24px'}}>
          ...
          <p style={{fontSize:12,color:sub,lineHeight:1.7,marginBottom:12}}>
            {es
              ? 'OLIVIA opera en la intersección de tres mercados regulados que convergen en 2026-2027: el EU ETS marítimo que obliga a navieras, CORSIA que convierte a las aerolíneas en los mayores compradores de créditos del mundo, y el CBAM que exige a los exportadores argentinos certificar su huella para competir en la UE.'
              : 'OLIVIA operates at the intersection of three regulated markets converging in 2026-2027: maritime EU ETS obligating shipping companies, CORSIA converting airlines into the world\'s largest credit buyers, and CBAM requiring Argentine exporters to certify their footprint to compete in the EU.'}
          </p>
  ...
  console.log('OK institucional: sección grandes emisores globales agregada');
} else {
  console.log('-- institucional: ya tiene la sección');
}

// ═══ 5. ACTUALIZAR FOOTER LANDING CON /grandes-emisores ═══
landing = fs.readFileSync('app/page.tsx', 'utf8');
if (!landing.includes("h:'/grandes-emisores'")) {
  landing = landing.replace(
  ...
  fs.writeFileSync('app/page.tsx', landing);
  console.log('OK landing footer: /grandes-emisores agregado');
}
