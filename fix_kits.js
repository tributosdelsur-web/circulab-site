    ...
    <div style={{minHeight:'100vh',background:bg,color:text,fontFamily:'Inter,system-ui',transition:'all 0.3s'}}>
        ...
        <div style={{maxWidth:900,margin:'0 auto'}}>
          ...
          <div style={{textAlign:'center',marginTop:20}}>
            <div style={{fontSize:12,color:sub,marginBottom:8}}>
              {es?'¿No encontrás tu kit? Contanos tu caso y te armamos una propuesta a medida.':'Can\'t find your kit? Tell us your case and we\'ll create a custom proposal.'}
            </div>
      ...
      <footer style={{borderTop:'1px solid '+border,padding:'24px',textAlign:'center'}}>
        ...
        <div style={{marginTop:8,display:'flex',gap:16,justifyContent:'center',flexWrap:'wrap'}}>
          ...
          <a href="/terminos" style={{fontSize:11,color:sub,textDecoration:'none'}}>{es?'Términos':'Terms'}</a>
        </div>
      </footer>
...
fs.writeFileSync('app/kits/page.tsx', page);
console.log('OK app/kits/page.tsx creada con 10 kits y 12 imágenes');

// Actualizar sitemap
if (fs.existsSync('public/sitemap.xml')) {
  let sitemap = fs.readFileSync('public/sitemap.xml', 'utf8');
  if (!sitemap.includes('/kits')) {
    sitemap = sitemap.replace(
      '</urlset>',
      `  <url>
    <loc>https://oliviacirculab.com.ar/kits</loc>
    ...
    console.log('OK sitemap.xml actualizado con /kits');
  }
}

// Agregar link a /kits en el footer del landing
let landing = fs.readFileSync('app/page.tsx', 'utf8');
if (!landing.includes("'/kits'") && !landing.includes('"/kits"')) {
  landing = landing.replace(
    "{l:'One Pager',h:'/onepager',c:'#22c55e'},",
    "{l:'One Pager',h:'/onepager',c:'#22c55e'},\n            {l:es?'Kits':'Kits',h:'/kits',c:'#22c55e'},"
  );
  fs.writeFileSync('app/page.tsx', landing);
  console.log('OK /kits agregado al footer del landing');
}

console.log('');
console.log('Listo. Próximo paso: npm run build');
