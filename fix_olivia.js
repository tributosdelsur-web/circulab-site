/* ============================================================
   OLIVIA · Deploy unico
   1) Crea /mapa (mapa de organicos + RAEE de CABA, sin nodos privados)
   2) Crea /raee (vertical de residuos electronicos)
   3) Enlaces: /ciudadano · /institucional · /consorcios · /metamorfosis · landing
   4) Sitemap
   ============================================================ */
const fs = require('fs');
...
/* ---------- 1. PAGINA /mapa ---------- */
if (!has('app/mapa')) fs.mkdirSync('app/mapa', { recursive: true });

const mapaPage = `'use client'
    ...
    <div style={{minHeight:'100vh',background:bg,color:text,fontFamily:'Inter,system-ui',transition:'all .3s'}}>
      ...
      <footer style={{borderTop:'1px solid '+border,padding:24,textAlign:'center'}}>
        ...
        <div style={{marginTop:8,display:'flex',gap:16,justifyContent:'center',flexWrap:'wrap'}}>
          ...
          <a href="/raee" style={{fontSize:11,color:sub,textDecoration:'none'}}>RAEE</a>
...
`;
write('app/mapa/page.tsx', mapaPage);
log('OK  app/mapa/page.tsx');

/* ---------- 2. PAGINA /raee ---------- */
if (!has('app/raee')) fs.mkdirSync('app/raee', { recursive: true });

const raeePage = `'use client'
...
`;
write('app/raee/page.tsx', raeePage);
log('OK  app/raee/page.tsx');

/* ---------- 3. ENLACES ---------- */

// helper: insertar bloque antes de un marcador
function insertBefore(file, marker, block, label) {
  if (!has(file)) { log('--  ' + file + ' no existe'); return false; }
  let c = read(file);
  if (c.includes(block.trim().slice(0, 40))) { log('--  ' + label + ': ya presente'); return false; }
  if (!c.includes(marker)) { log('!!  ' + label + ': no encontre "' + marker + '" · insertar a mano'); return false; }
...
// /ciudadano → puerta principal al mapa
insertBefore('app/ciudadano/page.tsx', '      {/* CTA FINAL */}', `
      {/* MAPA CABA */}
      ...
      </section>`, '/ciudadano · bloque mapa');

// /consorcios → link corto
insertBefore('app/consorcios/page.tsx', '      {/* CTA FINAL */}', `
      {/* LINK MAPA */}
      ...
      </section>`, '/consorcios · link mapa');

// /institucional → prueba de inteligencia territorial
insertBefore('app/institucional/page.tsx', '      {/* GRANDES EMISORES GLOBALES */}', `
      ...
      </section>`, '/institucional · bloque inteligencia territorial');

// /metamorfosis → salida al final
insertBefore('app/metamorfosis/page.tsx', '      {/* CTA FINAL */}', `
      {/* LINK MAPA */}
      ...
      </section>`, '/metamorfosis · link mapa');

/* ---------- 4. LANDING: puerta RAEE + link mapa ---------- */
if (has('app/page.tsx')) {
  let landing = read('app/page.tsx');
  let changed = false;

  if (!landing.includes('/raee')) {
    const m = landing.match(/<a href="\/grandes-generadores"[^>]*>/);
    if (m) {
      const puerta = `<a href="/raee" style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'20px',borderRadius:16,background:'linear-gradient(135deg,rgba(147,51,234,0.12),rgba(147,51,234,0.05))',border:'2px solid rgba(147,51,234,0.4)',textDecoration:'none'}}>
          ...
          `;
      landing = landing.replace(m[0], puerta + m[0]);
      changed = true;
      log('OK  landing · puerta RAEE');
    } else log('!!  landing: no encontre el ancla de /grandes-generadores · agregar puerta RAEE a mano');
  } else log('--  landing: ya tiene /raee');

  if (!landing.includes("h:'/mapa'")) {
    const anchors = ["{l:es?'Grandes Gen.':'Large Gen.',h:'/grandes-generadores',c:'#ef4444'},","{l:'RAEE',h:'/raee',c:'#9333ea'},"];
    let done = false;
    for (const a of anchors) {
      if (landing.includes(a)) {
        landing = landing.replace(a, a + "\n            {l:'RAEE',h:'/raee',c:'#9333ea'},\n            {l:es?'Mapa':'Map',h:'/mapa',c:'#0284c7'},");
        done = true; changed = true;
        log('OK  landing · links footer (RAEE + Mapa)');
        break;
      }
    }
    if (!done) log('!!  landing: no encontre el footer de links · agregar a mano');
  }

  if (changed) write('app/page.tsx', landing);
}
...
if (has('public/sitemap.xml')) {
  ...
  let add = '';
  if (!sm.includes('/mapa')) add += '  <url>\\n    <loc>https://oliviacirculab.com.ar/mapa</loc>\\n    <changefreq>monthly</changefreq>\\n    <priority>0.8</priority>\\n  </url>\\n';
  if (!sm.includes('/raee')) add += '  <url>\\n    <loc>https://oliviacirculab.com.ar/raee</loc>\\n    <changefreq>monthly</changefreq>\\n    <priority>0.8</priority>\\n  </url>\\n';
  if (add) { write('public/sitemap.xml', sm.replace('</urlset>', add + '</urlset>')); log('OK  sitemap'); }
  else log('--  sitemap: ya actualizado');
}

log('\\n=== Listo. Corriendo build... ===');
